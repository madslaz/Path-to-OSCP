...
* **Tast 3 and onwards**: Pivoted to xfreerdp with the credentials found using the file from task 2. Then used `reg save HKLM\* *.bak` for the three registries. Once the hives were backed up and transferred to the attacking machine, I did `impacket-secretsdump -sam SAM.bak -system SYSTEM.bak -security SECURITY.bak LOCAL`. 
